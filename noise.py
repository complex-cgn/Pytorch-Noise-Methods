...
class Noise:
    ...
    def white_noise_2d(self) -> torch.Tensor:
        """
        ...
        """
        if self.grayscale:
            return torch.rand(self.height, self.width, device=self.device)
        return torch.rand(3, self.height, self.width, device=self.device)
...
def generate_and_save_noise(
    ...
    # Generate noise
    noise = noise_generator.perlin_octave_2d(octaves)
...
def benchmark_noise_generation(
    ...
    logger.info(f"GPU time: {elapsed_time_ms:.2f} ms")
